...
| Placeholder | Description | Example |
|---|---|---|
| `{{DEFAULT_MODEL}}` | Preferred Copilot model setting (fallback when no per-agent model is set) | `claude-sonnet-4-6` |
| `{{MODEL_LIGHTWEIGHT}}` | Model for cheap phases: explore and verify at Level 1–2. Optimizes token cost for read-only and mechanical tasks. | `claude-haiku-4-5-20251001` / `gpt-4o-mini` |
| `{{MODEL_STANDARD}}` | Model for review phase and explore/verify at Level 3. Balances cost and reasoning for quality checks. | `claude-sonnet-4-6` / `gpt-4o` |
| `{{MODEL_ADVANCED}}` | Model for plan and implement phases at Level 2–3. Maximum reasoning for complex implementation decisions. | `claude-opus-4-7` / `gpt-4o` |
...
| Placeholder | Description | Example |
|---|---|---|
| `{{SAST_COMMAND}}` | Command to run static security analysis (Shift-Left gate, called by Implementer before each step completes). Leave empty string `""` if no SAST tool is available — the gate will be skipped gracefully. | `sh .github/scripts/security/codeql-scan.sh` or `python3 .github/scripts/security/sast-api.py` |
| `{{SAST_SEVERITY_THRESHOLD}}` | Minimum severity that causes SAST to block the pipeline | `high` (default) / `critical` / `medium` |
| `{{PRIMARY_LANGUAGE_CODEQL}}` | CodeQL language identifier for the project | `javascript` / `python` / `java` / `cpp` / `csharp` / `go` / `ruby` / `swift` |
